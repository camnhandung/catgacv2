...
def report_incident(
    ...
    # ✅ SỬA LỖI: lưu thời gian hiện tại theo giờ Việt Nam
    vn_tz = pytz.timezone("Asia/Ho_Chi_Minh")
...
def delete_incident(
    ...
    incident = session.get(IncidentReport, incident_id)
...
def get_incidents(
    ...
    for r in reports:
        ...
        # ✅ SỬA LỖI: convert thời gian sang múi giờ Việt Nam trước khi lọc và trả về
        if r.created_at.tzinfo is not None:
            created_at_vn = r.created_at.astimezone(vn_tz)
        else:
            # Nếu không có tzinfo (naive datetime), coi như UTC rồi convert sang VN
            created_at_vn = pytz.utc.localize(r.created_at).astimezone(vn_tz)

        if month and created_at_vn.month != month:
            continue

        if year and created_at_vn.year != year:
            continue
        ...
        results.append({
            ...
            "reason": r.reason,
            # ✅ SỬA LỖI: trả về thời gian đã được convert sang giờ Việt Nam
            "created_at": created_at_vn.isoformat(),
            "post_name": post_name,
